...
dataDir = '/home/anngic/dataset/data_coco'  # COCO数据集所在的路径
dataType = 'val5'  # 要转换的COCO数据集的子集名
annFile = '%s/annotations/instances_%s.json' % (dataDir, dataType)  # COCO数据集的标注文件路径
classes = get_classes_and_index('/home/anngic/luoyushan/darknet/data/coco_anngic.names')
   ...
   # print('annIds :%s' % annIds)
    for annId in annIds:
        ...
        if cat in classes:
            ...
            bb = convert(size, box)
            if True:
            # if ((cat=='person') and (bb[2]*bb[3] >= 0.1)) or ((cat!='person')&(bb[2]*bb[3] >= 0.0025)):# 筛选出w*h>=0.01的样本和h/w>=1/6的样本
    ...
    if objCount > 0:
        list_file.write('%s/%s/JPEGImages/%s\n' % (dataDir, dataType, filename))
                    #list_file.write(person2017_paths[num] + '\n')#从person2017中随机选取一张图片加入训练集
